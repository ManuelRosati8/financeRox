...
import { X, RefreshCw, PiggyBank } from "lucide-react";
...
// Keep editing resilient for legacy transactions that still contain hashtag suffixes.
              ...
              {goalAllocEnabled && (
                <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <div>
                    ...
                    <AppSelect
                      options={[
                        { value: "", label: "Scegli obiettivo..." },
                        ...goals.map((g) => ({
                          value: g.id,
                          label: `${g.icon || "🎯"} ${g.name} — ${new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR", maximumFractionDigits: 0 }).format(g.current_amount)} / ${new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR", maximumFractionDigits: 0 }).format(g.target_amount)}`,
                        })),
                      ]}
                        value={goalAllocId}
                      onChange={setGoalAllocId}
                    />
                  </div>
